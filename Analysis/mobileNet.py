  ...
  with tf.variable_scope(scope, 'MobilenetV1', [inputs, num_classes],
    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      with tf.variable_scope('Logits'):
        feature_maps_avg = tf.reduce_mean(tf.nn.relu(net), axis=(0,3),keep_dims=True)
        # change
        salient_objects = tf.stop_gradient(tf.squeeze(tf.nn.conv2d_transpose(feature_maps_avg,np.ones([51,51,1,1]).astype(np.float32), output_shape=[1,160,160,1], strides=[1,24,24,1],padding='VALID')))

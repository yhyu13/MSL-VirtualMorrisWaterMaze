...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer,grayScale=True):
        ...
        with tf.variable_scope(scope):
            ...
            self.turn = slim.fully_connected(rnn_out,3,
                activation_fn=tf.nn.softmax,
                weights_initializer=normalized_columns_initializer(0.01),
                biases_initializer=None)
            
            self.move = slim.fully_connected(rnn_out,2,
                activation_fn=tf.nn.softmax,
                weights_initializer=normalized_columns_initializer(0.01),
                biases_initializer=None)
            self.policy = tf.concat([self.turn,self.move],axis=1)
            ...
            if scope != 'global':
                self.actions_turn = tf.placeholder(shape=[None],dtype=tf.int32)
                self.actions_move = tf.placeholder(shape=[None],dtype=tf.int32)
                self.actions_onehot_turn = tf.one_hot(self.actions_turn,3,dtype=tf.float32)
                self.actions_onehot_move = tf.one_hot(self.actions_move,2,dtype=tf.float32)
                self.target_v = tf.placeholder(shape=[None],dtype=tf.float32)
                self.advantages = tf.placeholder(shape=[None],dtype=tf.float32)

                self.responsible_outputs_1 = tf.reduce_sum(self.turn * self.actions_onehot_turn, [1])
                self.responsible_outputs_2 = tf.reduce_sum(self.move * self.actions_onehot_move, [1])

                #Loss functions
                self.value_loss = 0.5 * tf.reduce_sum(tf.square(self.target_v - tf.reshape(self.value,[-1])))
                self.policy_loss = -tf.reduce_sum(tf.log(self.responsible_outputs_1)*self.advantages)-tf.reduce_sum(tf.log(self.responsible_outputs_2)*self.advantages)
...
class Worker():
    ...
    def train(self,rollout,sess,gamma,bootstrap_value):
        ...
        actions_turn,actions_move = rollout[:,1],rollout[:,2]
        rewards = rollout[:,3]
        ...
        feed_dict = {self.local_AC.target_v:discounted_rewards,
            ...
            self.local_AC.actions_turn:actions_turn,
            self.local_AC.actions_move:actions_move,
            self.local_AC.advantages:advantages,
            self.local_AC.state_in[0]:rnn_state[0],
            self.local_AC.state_in[1]:rnn_state[1]}
        v_l,p_l,e_l,g_n,_ = sess.run([self.local_AC.value_loss,
            self.local_AC.policy_loss,
            self.local_AC.entropy,
            self.local_AC.grad_norms,
            self.local_AC.apply_grads],
            feed_dict=feed_dict)
        return np.mean(v_l / len(rollout)),np.mean(p_l / len(rollout)),np.mean(e_l / len(rollout)), g_n
        ...
        with sess.as_default(), sess.graph.as_default():
            ...
            while not coord.should_stop():
                ...
                while self.env.is_episode_finished() == False:
                    ...
                    a_move = np.argmax(a_move == np.random.choice(a_move,p=a_move))
                    a = np.asarray([a_turn,a_move])
                    self.env.make_action(a_turn,100,a_move)
                    #sleep(0.01)
                    ...
                    episode_buffer.append([s,a_turn,a_move,r,s1,d,v[0,0]])
                ...
                if len(episode_buffer) != 0:
                    if self.is_training:
                        v_l,p_l,e_l,g_n = self.train(episode_buffer,sess,gamma,0.0)
                ...
                if episode_count % 5 == 0 and episode_count != 0:
                    ...
                    if episode_count % 500 == 0 and self.name == 'worker_0' and self.is_training:

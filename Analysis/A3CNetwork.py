...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer):
        with tf.variable_scope(scope):
            ...
            if scope == 'worker_0':
                feature_maps_avg2 = tf.reduce_mean(self.conv2, axis=3,keep_dims=True)
                #print(feature_maps_avg2.get_shape())
                #print(self.conv2.get_shape())
                feature_maps_avg1 = tf.reduce_mean(self.conv1, axis=3,keep_dims=True)
                #print(feature_maps_avg1.get_shape())
                #print(self.conv1.get_shape())
                scale_up_deconv2 = tf.nn.conv2d_transpose(feature_maps_avg2,np.ones([4,4,1,1]).astype(np.float32),[-1,39,39,1],strides=[2,2],padding='VALID')
                #print(scale_up_deconv)
                self.salient_objects = tf.nn.conv2d_transpose(tf.multiply(feature_maps_avg1,scale_up_deconv2),np.ones([8,8,1,1]).astype(np.float32),[-1,84,84,1],strides=[2,2],padding='VALID')
                
            
            hidden = slim.fully_connected(slim.flatten(self.conv2),256,activation_fn=tf.nn.elu)
            
            #Recurrent network for temporal dependencies
            lstm_cell = tf.contrib.rnn.LayerNormBasicLSTMCell(256,dropout_keep_prob=1.0)

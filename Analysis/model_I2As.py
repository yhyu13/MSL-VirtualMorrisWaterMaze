...
class AC_Network():
    def __init__(self,s_size,a_size,scope,trainer,noisy,grayScale):
        with tf.variable_scope(scope):
            ...
            hidden = tf.nn.tanh(logits)'''
            hidden = slim.fully_connected(self.conv6,256,
        ...
        with sess.as_default(), sess.graph.as_default():
            ...
            while not coord.should_stop():
                ...
                while self.env.is_episode_finished() == False:
                    ...
                    if len(episode_buffer) == 5 and d != True and episode_step_count != max_episode_length - 1: # change pisode length to 5, and try to modify Worker.train() function to utilize the next frame to train imagined frame.

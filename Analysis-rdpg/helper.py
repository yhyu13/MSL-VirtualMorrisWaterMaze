...
def update_target_netowkr(TAU):
	net_actor = tf.get_collection(tf.GraphKeys.TRANINABLE_VARIABLES, 'global/actor')
	net_critic = tf.get_collection(tf.GraphKeys.TRANINABLE_VARIABLES, 'global/critic')
	net_actor_target = tf.get_collection(tf.GraphKeys.TRANINABLE_VARIABLES, 'global/actor/target')
	net_critic_target = tf.get_collection(tf.GraphKeys.TRANINABLE_VARIABLES, 'global/critic/target')
	update_target = []
...
def process_salient_object(s):
    ...
    s = np.clip(s / (np.max(s)+1e-2), 0.05, 0.95)
